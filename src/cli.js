...
const commands = {};
let systemData = {};
const rootPath = 'users/ray-marvin/root';
const user = 'ray-marvin';
...
commands.ls = (directory) => {
    ...
    if (directory === '-l') {
        return systemData[getDirectory() + 'List'];
    }
...
$(() => {
    ...
    pages.push($.get('pages/rootList.html'));
    pages.push($.get('pages/skills.html'));
    pages.push($.get('pages/skillsList.html'));
    $.when
        .apply($, pages)
        .done(
            (
                ...
                systemData['rootList'] = rootListData[0];
                systemData['skills'] = skillsData[0];
                systemData['skillsList'] = skillsListData[0];
            },
        );
